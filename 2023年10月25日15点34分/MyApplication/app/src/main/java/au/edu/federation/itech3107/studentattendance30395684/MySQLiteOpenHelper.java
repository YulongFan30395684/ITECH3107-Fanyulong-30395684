...
public class MySQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREAT_COURSE_TABLE_SQL);
        db.execSQL(CREAT_STUDENT_COURSE_TABLE_SQL);
    ...
    public ArrayList<String> showAttendedStudents(String courseName, int nowWeek){
        SQLiteDatabase db=getWritableDatabase();

        System.out.println("maopaoAttendance+ "+courseName+" "+nowWeek);

        String whereClause = "courseName" + "='"+courseName+"' AND " + ("week"+nowWeek) + "='" + "1"+ "'";
        String s1="select * from "+TABLE_NAME_STUDENT_COURSE+" where courseName= \""+courseName+"\" and "+"week"+nowWeek+" = 1";
//
//        select * from sc where courseName=  "ITECH3002" and week7 = 1
//        db.execSQL(s1);
        Cursor cursor = db.rawQuery(s1,null);
//        Cursor cursor = db.query(TABLE_NAME_STUDENT_COURSE, null, whereClause, null, null, null, null);

        ArrayList<String> studentAttendCourseList=new ArrayList<>();
        if (cursor!=null){
            while(cursor.moveToNext()){
                String studentName=cursor.getString(2);
                studentAttendCourseList.add(studentName);
            }
        }

        System.out.println("maopaoAttendance "+studentAttendCourseList.size());

        System.out.println("weism"+s1);

        for (int i = 0; i < studentAttendCourseList.size(); i++) {
            System.out.println("maopaoAttendance "+studentAttendCourseList.get(i));
        }
        return studentAttendCourseList;
